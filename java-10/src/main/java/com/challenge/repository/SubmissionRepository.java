...
public interface SubmissionRepository extends JpaRepository<Submission, SubmissionId> {
    ...
    @Query(" select max(sub.score) from Submission sub join sub.id.challenge cha " +
            " where cha.id = :challengeId ")
    Optional<BigDecimal> findHigherScoreByChallengeId(@Param("challengeId") Long challengeId);

    @Query(" select sub from Submission sub join sub.id.challenge cha " +
            " join cha.accelerations acc where acc.id = :accelerationId and cha.id = :challengeId ")
